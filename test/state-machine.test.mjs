...
import { BLACKMAGIC_APPLIED_NOTICE, BLACKMAGIC_MODEL_SUMMARY, BLACKMAGIC_READY, COMPACTION_DECISION, COMPACTION_OUTCOME, CONTINUATION_REPLAY, LEGACY_BLACKMAGIC_MODEL_SUMMARIES, PROVIDER_MISMATCH, STATE_MACHINE, SUMMARY_STAYS_READABLE, classifyContinuation, continuationFooter, decideCompaction, isBlackmagicModelPlaceholder, projectBlackmagicStatus } from "../src/state-machine.mjs";
...
test("the executable machine exports its exact states and complete 3x3 transitions", () => {
  ...
  assert.equal(BLACKMAGIC_APPLIED_NOTICE, "Server-side compaction applied. Keep this model and provider.");
  assert.equal(isBlackmagicModelPlaceholder(BLACKMAGIC_MODEL_SUMMARY), true);
  for (const summary of LEGACY_BLACKMAGIC_MODEL_SUMMARIES) assert.equal(isBlackmagicModelPlaceholder(summary), true);
  assert.equal(isBlackmagicModelPlaceholder("readable legacy summary"), false);
...
test("Pi's native compaction component never receives human-only Blackmagic advice", () => {
  initTheme("dark");
  ...
  assert.doesNotMatch(plain(), /Server-side compaction applied|Keep this model and provider/);
...
test("compaction decisions delegate, attempt, cancel, or apply without side effects", () => {
  ...
  const compaction = { summary: BLACKMAGIC_MODEL_SUMMARY };
